...
import { Award, Flame, Navigation, Clock, X, LogOut, MapPin, Shield, Users, RefreshCw } from 'lucide-react';
import { clearSession, getStoredUser } from '../services/authService.js';
import wsService from '../services/wsService.js';

export function UserProfileModal({ isOpen, onClose, profileData, onLogout, isOwnProfile = true }) {
  ...
  const data = profileData || {
    ...
    email: user.login ? `${user.login}@uram.ru` : '',
    player_color_hex: '#2563eb',
    avatar_url: '',
    team_name: 'URAM Community',
    team_tag: 'URAM',
    team_color_hex: '#2563eb',
    team_avatar_url: '',
  ...
  const distanceKm = ((Number(data.total_distance_meters) || 0) / 1000).toFixed(2);
  ...
  const handleRefreshProfile = () => {
    wsService.requestUserProfile(data.user_id || 0);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in select-none">
      ...
      >
        ...
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${data.team_color_hex || data.player_color_hex || '#fe4a09'} 0%, #2563eb 100%)`
          }}
        >
          <div className="absolute top-3 left-4 flex items-center gap-2">
            ...
            <span className="text-[11px] font-heading font-black tracking-tight text-white">
              {isOwnProfile ? 'МОЙ ПРОФИЛЬ' : 'ПРОФИЛЬ АТЛЕТА'} • RUN URAM KAZAN
            </span>
          </div>
          ...
          <div className="flex items-center gap-3.5 z-10 translate-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/90 font-medium">
                {data.team_name || 'URAM Community Runner'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-8 px-6 pb-6 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
              ...
              <div className="text-lg font-bold text-slate-900 font-mono">{data.total_runs || 0}</div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
              ...
              <div className="text-lg font-bold text-blue-600 font-mono">
                {data.current_held_hexagons || 0} <span className="text-xs font-normal text-slate-500">сот</span>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-orange-50/70 border border-orange-200 col-span-2 sm:col-span-2">
              ...
              <div className="text-xl font-black text-orange-600 font-mono">
                {data.total_uram_points || 0} <span className="text-xs font-normal text-orange-500">pts</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50/70 border border-slate-200 text-xs space-y-2.5">
            <div className="flex items-center justify-between text-slate-600">
              ...
              <div className="flex items-center gap-1.5 font-bold text-slate-900">
                ...
                <span>{data.team_name || 'Без команды'}</span>
              </div>
            </div>
            <div className="flex items-center justify-between text-slate-600">
              <span>Протокол / Схема:</span>
              <span className="font-mono text-blue-600 font-semibold">user.UserProfileResponse</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition cursor-pointer"
            >
              <RefreshCw size={13} />
